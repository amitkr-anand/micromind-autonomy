...
# ---------------------------------------------------------------------------
# Vehicle B position log synthesiser
# ---------------------------------------------------------------------------

# Nav mode per phase for Vehicle B (MicroMind)
_VEH_B_NAV_MODE = {
    1: "GNSS_AIDED",
    2: "VIO_PRIMARY",
    3: "VIO_PRIMARY",
...
def _build_vehicle_b_position_log(
    schedule: DisturbanceSchedule,
    max_km: float,
    record_cadence_km: float = 0.2,
) -> list:
    """
    Synthesise a position log for Vehicle B following the nominal planned route.

    Vehicle B (MicroMind) stays close to the planned route (NAV-01 compliant).
    Position records are emitted every record_cadence_km along the route.

    Fields match demo_data_pipeline.get_vehicle_tracks() contract:
      sim_timestamp_ms, mission_km, north_m, east_m, alt_m,
      nav_mode, phase, gnss_available
    """
    route = build_nominal_route()
    scenario = BCMP2Scenario()

    log = []
    km = 0.0
    while km <= max_km:
        t_s = km * 1000.0 / VEHICLE_SPEED_MS
        sim_ts_ms = int(round(t_s * 1000))

        wp = scenario.interpolate_waypoint(km)
        phase = schedule.phase_at_km(km)
        gnss_ok = schedule.gnss_available(t_s)
        nav_mode = _VEH_B_NAV_MODE.get(phase, "VIO_PRIMARY")
        if gnss_ok:
            nav_mode = "GNSS_AIDED"

        # Altitude: interpolate from route waypoints
        alt_m = wp.alt_m

        log.append({
            "sim_timestamp_ms": sim_ts_ms,
            "mission_km":       round(km, 4),
            "north_m":          round(wp.north_m, 2),
            "east_m":           round(wp.east_m, 2),
            "alt_m":            round(alt_m, 1),
            "nav_mode":         nav_mode,
            "phase":            phase,
            "gnss_available":   gnss_ok,
        })
        km += record_cadence_km
...
def run_bcmp2(
    ...
    # Vehicle B synthesised position log — follows planned route with bounded
    # drift (NAV-01 compliant).  bcmp1_runner is frozen and produces no
    # position log; this synthesised track is Orchestration Box only and must
    # not be used as algorithm-level evidence.
    veh_b_pos_log = _build_vehicle_b_position_log(schedule, max_km)
    ...
    output = {
        ...
        # VIZ-02: position logs for Run 2 animation.
        # vehicle_a.position_log is generated by baseline_nav_sim.to_kpi_dict().
        # vehicle_b_position_log is synthesised (bcmp1_runner frozen, no pos track).
        "vehicle_b_position_log": veh_b_pos_log,
    }
