...
# GNSSSpoofInjector not required — GNSS loss is driven by gnss_active phase flag
...
def _make_gnss_measurement(
    ...
    """
    Build a GNSSMeasurement for the BIM.

    Exact field schema (core/bim/bim.py S2):
        pdop:                   Optional[float]
        cn0_db:                 Optional[float]
        tracked_satellites:     Optional[int]
        gps_position_enu:       Optional[np.ndarray]  shape (3,) ENU metres
        glonass_position_enu:   Optional[np.ndarray]  shape (3,) ENU metres
        doppler_deviation_ms:   Optional[float]
        pose_innovation_m:      Optional[float]
        ew_jammer_confidence:   float  (default 0.0)
    """
    rng = np.random.default_rng(int(ground_track_m) % 2**31)
    pos_enu = np.array([true_east_m, true_north_m, 0.0])
    ...
    if gnss_active:
        return GNSSMeasurement(
            ...
            gps_position_enu     = pos_enu + rng.normal(0, 1.5, 3),
            glonass_position_enu = pos_enu + rng.normal(0, 2.0, 3),
            doppler_deviation_ms = rng.uniform(0, 0.05),
            pose_innovation_m    = rng.uniform(0, 1.5),
            ew_jammer_confidence = 0.0,
        )
    else:
        # Jammed: degraded PDOP, low C/N0, large GPS spoof offset,
        # GLONASS unaffected (drives inter-constellation delta), high pose innovation
        spoof_offset = np.array([rng.uniform(50, 120), rng.uniform(50, 120), 0.0])
        return GNSSMeasurement(
            pdop                 = 8.0 + rng.uniform(0, 4.0),
            cn0_db               = 22.0 + rng.normal(0, 3.0),
            tracked_satellites   = 3,
            gps_position_enu     = pos_enu + spoof_offset,
            glonass_position_enu = pos_enu + rng.normal(0, 2.0, 3),
...
def run_nav_scenario(
    ...
    # GNSS phase is controlled directly by ground_track_m thresholds
    ...
    while ground_track_m <= TOTAL_DISTANCE_M:
        ...
        meas = _make_gnss_measurement(true_north, true_east,
                                      ground_track_m, gnss_active)
        bim_output = bim.evaluate(meas)
        # BIMOutput.trust_state is already BIMState (GREEN/AMBER/RED)
        bim_state_fsm = bim_output.trust_state
