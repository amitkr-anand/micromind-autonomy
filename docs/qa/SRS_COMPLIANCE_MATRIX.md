...
| D6 | D3 total timeout (10s) | Log OFFBOARD_UNRECOVERED. ABORT_MISS. | CLOSED | CLOSED | NO | IMPLEMENTED — OffboardRecoveryFSM (integration/bridge/offboard_recovery_fsm.py). D2 5s + D3 5s + D6 abort. OFFBOARD_UNRECOVERED event. abort_fn() callback on D6. 6a30295. SITL integration confirmed a3fd35e. | Tested and Verified | 4 tests / 17 assertions. D2 recovery, D3 recovery, full timeout abort, timing bounds. test_it_d6_timeout.py. 6a30295. IT-D6-SITL-01: 5 gates PASS. Live SITL 10.00s elapsed. OFFBOARD_UNRECOVERED total_elapsed_ms=10001ms, attempts_made=10. Link-severed semantics. a3fd35e. | test_it_d6_timeout.py; test_it_d6_sitl.py | Medium | Deputy 1 | None — SITL confirmed. | None | QA-061; QA-063 |
...
| §12 | RS-02 | Log Completeness and Rolling Policy | Medium | ALL AVP | CLOSED | CLOSED | NO | IMPLEMENTED — RollingMissionLog in logs/mission_log_schema.py. Roll on 200MB or 30min. LOG_ROLLED event. CRITICAL (P0) never dropped. dropped_records_total tracked. LOG_BUFFER_HIGH at 80%. | Tested and Verified — UT-RS-02 5/5 PASS. test_roll_on_size, test_roll_on_time, test_critical_never_dropped, test_dropped_count_tracked, test_log_rolled_event_fields. | RollingMissionLog wraps MissionLog pattern. CRITICAL_CATEGORIES = L10S_SE/MISSION_START/MISSION_END/STATE_TRANSITION. File format JSONL. GAP-09 CLOSED. | This commit | UT-RS-02 | Low | Deputy 1 | None — GAP-09 CLOSED | Baseline 547/547 | GAP-09 CLOSED |
| §12 | RS-03 | Watchdog and Restartability Classification | Medium | ALL AVP | PARTIAL | PARTIAL | NO | IMPLEMENTED — ProcessWatchdog (core/watchdog/process_watchdog.py). PROCESS_REGISTRY 6 entries. Decision logic tested. 3fc84bc. | Partially Tested — decision logic 6 tests / 30 assertions PASS (QA-063). ST-RESTART-01 real SIGKILL test Phase D. | PROCESS_REGISTRY covers NOT_RESTARTABLE (ESKFCore), RESTARTABLE_WITH_SHM (NavigationManager, MissionManager, PX4Bridge), RESTARTABLE_WITHOUT_SHM (EWManager, LogBus). UT-RS-03 PASS. | `3fc84bc` | UT-RS-03 | Low | Deputy 1 | ST-RESTART-01: real SIGKILL test Phase D (GAP-08) | Phase D: ST-RESTART-01 | QA-063; GAP-08 |
...
|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|
| RESTARTABLE | Watchdog-monitored processes (EW Manager, LogBus, NavigationManager, MissionManager, PX4Bridge) | Watchdog: PROC_RESTART. Restart ≤2s + restore ≤10s. ABORT_MISS after max_restart_attempts. | PARTIAL | PARTIAL | YES (QA-062/QA-063) | IMPLEMENTED — ProcessWatchdog (core/watchdog/process_watchdog.py). PROCESS_REGISTRY with 6 entries. D2/D3 SHM path + restart loop. 3fc84bc. | Partially Tested — decision logic tested (6 tests / 30 assertions). Real SIGKILL stimulus (ST-RESTART-01) remains Phase D. | ProcessWatchdog PROCESS_REGISTRY covers RESTARTABLE_WITHOUT_SHM (EWManager, LogBus) and RESTARTABLE_WITH_SHM (NavigationManager, MissionManager, PX4Bridge). Decision path tested by UT-RS-03 (QA-063). | UT-RS-03 | Low | Deputy 1 | ST-RESTART-01: real SIGKILL test Phase D. | Phase D: ST-RESTART-01 real SIGKILL test | GAP-08; QA-063 |
| NOT_RESTARTABLE | ESKF core (`core/ekf/error_state_ekf.py`) — frozen file | Watchdog: ABORT_MISS immediately on heartbeat miss. No restart. | CLOSED | PARTIAL | YES (QA-062/QA-063) | IMPLEMENTED — PROCESS_REGISTRY ESKFCore=NOT_RESTARTABLE. ESKF_CORE_FAILURE event + abort_fn() path in ProcessWatchdog. 3fc84bc. | Partially Tested — PROCESS_REGISTRY ESKFCore=NOT_RESTARTABLE confirmed. ESKF_CORE_FAILURE→abort_fn path tested. Direct SIGKILL stimulus (ST-RESTART-01) Phase D. | ESKF classified NOT_RESTARTABLE in ProcessWatchdog registry. ABORT_MISS path exercised by UT-RS-03 test_not_restartable_triggers_abort (QA-063). Frozen file protection prevents accidental modification. | FSM ABORT_MISS tests; UT-RS-03 | High | Deputy 1 | ST-RESTART-01 Phase D. | Phase D (ST-RESTART-01) | QA-063 |
...
| E-03 | Restartability Classification | Each process classified RESTARTABLE / NOT_RESTARTABLE. Watchdog respects classification. | PARTIAL | PARTIAL | YES (QA-062/QA-063) | IMPLEMENTED — PROCESS_REGISTRY classification in core/watchdog/process_watchdog.py. 6 processes classified. 3fc84bc. | Partially Tested — test_registry_classification_correct confirms all 6 classifications. ST-RESTART-01 Phase D. | ProcessWatchdog PROCESS_REGISTRY committed at QA-063 (core/watchdog/process_watchdog.py). 6 processes classified. UT-RS-03 test_registry_classification_correct asserts all 6 entries. | UT-RS-03 | Low | Deputy 1 | ST-RESTART-01: real SIGKILL test Phase D (GAP-08). | Phase D: ST-RESTART-01 | GAP-08; QA-063 |
| E-04 | Log Rolling Policy | Log files rolled on size/time threshold. dropped_count tracked. UT-RS-02 required. | CLOSED | CLOSED | NO | IMPLEMENTED — RollingMissionLog. Roll triggers: 200MB size OR 30min age. LOG_ROLLED on each roll. CRITICAL (P0) never dropped, written to critical_events.log. dropped_records_total + dropped_critical_events_count=0 invariant. LOG_BUFFER_HIGH at 80% capacity. JSONL file naming mission_log_<timestamp>.jsonl. GAP-09 CLOSED. | Tested and Verified — UT-RS-02 5/5 PASS. Size roll, time roll, critical non-drop, drop count, LOG_ROLLED fields all verified. | RollingMissionLog added to logs/mission_log_schema.py. CRITICAL_CATEGORIES frozenset defined. test_ut_rs02.py committed. Baseline 547/547. | UT-RS-02 | Low | Deputy 1 | None — GAP-09 CLOSED | SIL 547/547 | GAP-09 CLOSED |
...
| UT-RS-02 | Log Rolling Policy | RS-02 | §12 | ALL AVP | Unit | SIL | PASSED | This commit | 5/5 PASS — test_roll_on_size, test_roll_on_time, test_critical_never_dropped, test_dropped_count_tracked, test_log_rolled_event_fields | tests/test_ut_rs02.py | GAP-09 CLOSED | None | GAP-09 CLOSED |
| UT-RS-03 | Watchdog Restartability — Decision Logic | RS-03 | §12 | ALL AVP | Unit | SIL | PARTIAL — decision logic PASS, SIGKILL stimulus Phase D | QA-063 | 6 tests / 30 assertions. 3fc84bc. | `3fc84bc` | ST-RESTART-01 real SIGKILL Phase D | GAP-08; QA-063 |
...
| MAVLink interruption — D6 full 10s timeout → ABORT | PX4-01, EC-01, D1..D6 | IT-PX4-01 covers nominal reconnect. IT-D6-TIMEOUT-01 PASSED (SIL). IT-D6-SITL-01 PASSED — 5 gates, 10.00s elapsed, OFFBOARD_UNRECOVERED confirmed. a3fd35e. | CLOSED | `97b2f5a`, `6a30295`, `a3fd35e` | None. | None | QA-061; QA-063 |
