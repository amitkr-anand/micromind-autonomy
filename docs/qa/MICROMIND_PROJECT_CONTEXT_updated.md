...
**Last Updated:** 19 April 2026 (end of day — HIL H-1/H-2/H-3/H-4 complete, Sandbox Phase D complete)  
...
| Repo | Purpose | State |
|---|---|---|
| `amitkr-anand/micromind-autonomy` | Main autonomy stack | Gates 1–6 LOCKED, 483/483 certified baseline, HEAD 99b6421 |
...
**Certified baseline runner:** `bash run_certified_baseline.sh` — target 483/483 on both dev and Orin
**Total regression baseline:** 483 tests (479 legacy + 4 Gate 6 Jammu-Leh additions)

---

## 6. Current Programme State (19 April 2026 — end of day)

### HIL Phase — Orin Nano Super (19 April 2026)

| Gate | Status | Result | Commit |
|------|--------|--------|--------|
| H-1 Environment | ✅ PASS | 483/483 on Orin, Python 3.11, terrain 415MB min set | `e0eb921` |
| H-2 Certified baseline (embedded H-1) | ✅ PASS | 483/483 at max clocks, 18m25s runtime | `e0eb921` |
| H-3 LightGlue GPU latency | ✅ PASS | 628ms median, 1630ms P99, 45× inside 74,000ms budget | `b3a8c77` |
| H-4 LightGlue IPC bridge | ✅ FULL PASS | Unix socket, T1/T2/T3 PASS, conf=0.743 real GPU Site 04 | `99b6421` |

**OI-25 CLOSED:** ESKF P99=0.1136ms at max clocks. Budget 50ms, margin 99.8%. Orin Nano sufficient — no Orin NX escalation.

**Python version note:** Jetson PyTorch wheels cp310 (Python 3.10) only. Production env is Python 3.11. Resolution: LightGlue runs in separate `hil-h3` process via Unix socket IPC bridge at `integration/lightglue_bridge/`. Interface: `docs/interfaces/L2_LIGHTGLUE_IPC.md`.

**SAL-1 prerequisite:** `search_pad_px = 25` (125m fixed) in `core/ins/trn_stub.py:56` — will be replaced with dynamic ESKF covariance-derived radius in Gate 7.

### Sandbox Phase D — UAV-VisLoc (Xu et al. arXiv:2405.11936) — COMPLETE

| Phase | Status | Key finding | Commit |
|-------|--------|-------------|--------|
| D-1 Baseline | ✅ COMPLETE | LightGlue 56/60 (93%), 42.4m mean GT error | `f74bd82` |
| D-2 Resolution degradation | ✅ COMPLETE | Min viable satellite ≤3m/px; Sentinel-2 10m/px fails | `aad1fe3` |
| D-3 Robustness | ✅ COMPLETE | VIO heading budget ±10°; FOV 60° optimal | `3339858` |
| LightGlue roles 2a/2b | ✅ REJECTED | Latency (role 2a) and measurement instability (role 2b) | `f53d951` |

**Consolidated LightGlue parameters (AD-23):** conf≥0.35 | FOV 60° | heading ±10° | satellite ≤3m/px | 1280px working res | structured terrain only
...
| HIL H-4 — LightGlue Subprocess IPC Bridge | ✅ COMPLETE | Unix socket IPC: server.py (hil-h3 Python 3.10) + client.py (Python 3.11). Orin verified with real Site 04 frame (04_0001.JPG): T1 PASS (0.7ms), T2 PASS (conf=0.743 correction=93.9m match_ms=3192ms), T3 PASS. LIGHTGLUE_EXTRA_TILES registers satellite04.tif (119.906–119.955°E / 32.151–32.254°N). Contract: docs/interfaces/L2_LIGHTGLUE_IPC.md. | `f0d7cc3` |
...
| ~~OI-25~~ **CLOSED** HIL H-1/H-3 19 Apr 2026 — ESKF P99=0.1136ms on Orin Nano Super (max clocks). Budget 50ms, margin 99.8%, 440× inside budget. Orin Nano sufficient. | Testing | CLOSED |
...
| OI-47 | SAL-1 (Segment Awareness Layer Phase 1): replace fixed SEARCH_PAD_PX=25 in core/ins/trn_stub.py with dynamic radius from ESKF position covariance. Gate 7 candidate. No frozen file modifications required. | Architecture | MEDIUM — Gate 7 |
| OI-48 | HIL H-5: End-to-end L2 correction integration — wire lightglue_client.match() into NavigationManager.update_trn() and verify correction applied to ESKF on Shimla corridor replay on Orin. | Testing | MEDIUM — next HIL session |
| OI-49 | SAL-2: terrain-class-specific LightGlue confidence thresholds (ACCEPT=0.35, CAUTION=0.40, SUPPRESS=skip). Driven by existing terrain_suitability_score per corridor segment. Gate 7 candidate. | Architecture | MEDIUM — Gate 7 |
---
