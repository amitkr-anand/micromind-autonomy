...
## AD-23 — L2 Matching Architecture: LightGlue as Primary, LoFTR as Fallback
**Date:** 18–19 April 2026
**Status:** VALIDATED
**Owner:** Deputy 1 (Architect Lead)

### Decision
The L2 absolute position reset layer uses SuperPoint+LightGlue as the primary matcher and LoFTR as the fallback. Phase correlation is retained only for legacy SIL gate compatibility and is not the operational L2 mechanism.

| Layer | Mechanism | Condition | Status |
|-------|-----------|-----------|--------|
| L2 Primary | SuperPoint+LightGlue | Structured terrain (roads, buildings, field boundaries), 400–800m AGL | VALIDATED |
| L2 Fallback | LoFTR dense matching | Mixed/agricultural/forest terrain, higher altitudes | CHARACTERISED |
| L2 Legacy | Phase correlation NCC | SIL gate compatibility only | NOT OPERATIONAL |
| L2 Future | SuperPoint+LightGlue at ≤3m/px | Requires CARTOSAT-1 or equivalent | DEFERRED |

### Validated Performance (UAV-VisLoc dataset, Site 04, Xu et al. 2024)
- Accept rate: 56/60 frames (93%) at confidence threshold ≥0.35
- Mean GT error: 42.4m at 540m AGL
- P90 GT error: 78.9m
- Latency: 72ms (RTX 5060 Ti); estimated 300–500ms (Orin Nano)
- Reference satellite resolution: 0.28m/px (Google Earth)
- UAV camera resolution: 3976×2652px (dataset); production: 1280×720px (TASL)

### Operational Envelope
- Works: Structured terrain with road/building geometry at 400–800m AGL
- Suppressed: Agricultural fields without road network, forest, >1500m AGL, desert, water
- Temporal change: Sites with significant scene change between satellite and UAV acquisition will fail — correct suppression behaviour, not matcher failure

### Confidence Threshold Calibration
Threshold set at 0.35 (Deputy 1, 19 April 2026). Previous value 0.50 rejected 8 frames with 80+ RANSAC inliers — geometrically sound matches incorrectly gated. At 0.35: 56/60 accepted (+2.4m mean error vs 0.50).

### LightGlue Role Evaluation (19 April 2026)
- Role 1 (EO-to-satellite position): VALIDATED — primary L2 mechanism
- Role 2a (EO-to-EO VIO heading support): REJECTED — 72–500ms incompatible with 20Hz VIO fast loop
- Role 2b (heading from inlier angles): REJECTED — 171° variance across 10 frames, reflects terrain edge geometry not UAV heading

### Implementation Requirements
- CRS-aware satellite crop extraction (EPSG:4326 degree vs projected metre)
- Heading compensation: rotate_to_north() applied AFTER downscaling to working resolution
- Multi-tile satellite lookup: per-frame tile selection required for sites with multiple tiles
- Pre-evaluation preflight: 00_dataset_preflight.py mandatory before any evaluation run

### References
- Sandbox commit f53d951 — LightGlue role evaluation notes
- Sandbox commit f74bd82 — confidence threshold 0.35
- UAV-VisLoc: Xu W. et al. (2024). arXiv:2405.11936
...
### Decision
The L2 matching layer shall be extended with a Segment Awareness Layer (SAL) that constrains EO matching to the expected segment of the mission corridor rather than treating it as a global retrieval problem. The SAL is implemented in three phases.

### Motivation
MicroMind missions are deterministic: launch point, corridor, altitude, heading, speed, and expected arrival time per waypoint are all known at mission start. The onboard problem is not "Where am I globally?" but "Given that I should be near Waypoint W on Segment N, what observable features confirm or correct my position?" This reduces search space, enables terrain-class-specific thresholds, and makes the L2 layer more robust in suppression zones.

### Three-Phase Implementation

**SAL-1 — Bounded search (implementable immediately against SIL baseline):**
Constrain NCC/LightGlue search area using INS prediction uncertainty ellipse. The search_pad_px parameter in the existing corridor simulation is the precursor. No new landmark pipeline required. Gate 7 candidate.

**SAL-2 — Terrain-class-specific acceptance thresholds (implementable immediately):**
The current global confidence threshold (0.35) applies uniformly. Terrain suitability scanner already classifies each km as ACCEPT/CAUTION/SUPPRESS. Per-segment threshold calibration from suitability scan reduces false suppressions on textured terrain and prevents false acceptances on featureless terrain. No frozen file modifications required.

**SAL-3 — Semantic landmark matching as SUPPRESS-zone fallback (new sandbox required):**
For the two documented SUPPRESS zones in the Jammu-Leh corridor (Kashmir valley km 60–120, Ladakh plateau km 300–330), semantic feature matching (canal networks, road intersections, field boundary patterns) may provide coarser but non-zero position corrections where NCC/LightGlue are suppressed. This is the novel research contribution.

Falsifiable question: "Can a landmark-aware matcher produce a usable EKF measurement in SUPPRESS-class terrain zones where NCC/LightGlue fail?"

### What the SAL Does NOT Do
- Heading correction: VIO (OpenVINS) remains the sole heading measurement source. LightGlue heading extraction was evaluated and rejected (AD-23, Role 2b). The L2 layer provides position correction only.
- Replace VIO: The SAL is a slow-loop position reset mechanism operating at 2–5km intervals, not a fast-loop (20Hz) replacement for optical flow.
- Solve temporal change: Scene change between satellite and UAV acquisition date is a fundamental limitation. The SAL does not address this — it relies on persistent geometric features (roads, ridges, water bodies) that survive temporal gaps.

### Segment Schema (per 5km corridor segment)
```
segment_record:
  passive_reference:   # Already in stack — satellite patch, DEM patch
    satellite_tile_path
    dem_tile_path
    terrain_suitability_score
    terrain_class: [urban, agricultural, forest, mountain, desert, water]
  active_landmarks:    # SAL-3 addition — not yet implemented
    primary:   [road_intersection, canal_network, field_boundary, ridgeline]
    fallback:  [village_cluster, river_bend, clearing]
  matching_params:
    confidence_threshold  # terrain-class calibrated (SAL-2)
    search_radius_m       # INS uncertainty ellipse (SAL-1)
    expected_heading_deg
    expected_altitude_m
```

### Consequences
- SAL-1 and SAL-2 are Gate 7 scope items — no new infrastructure
...
- Gate 6 findings: SUPPRESS zones km 60–120 and km 300–330 (GATE6_CORRIDOR_FINDINGS.md)
- LightGlue validation: AD-23
- External analysis: Segment-Anchored Visual Localization Hypothesis review, 19 April 2026
