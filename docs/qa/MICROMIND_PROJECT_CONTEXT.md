...
**Last Updated:** 18 April 2026 (QA-039)  
...
**Total regression baseline:** 479 tests (406 certified + 19 Gate 4 + 17 Gate 5 + 15 Gate 6 cross-modal + 22 Gate 6 Jammu-Leh)
...
| OI-46 | **UNRESOLVED (QA-038 revised classification).** QA-037 cross-modal finding revised: `sentinel_tci_dem_extent.tif` (167 MB) exists in simulation/terrain/shimla/ and was likely packed into the .blend scene. Frame R/B ratios (1.13–1.24) match Sentinel-2 TCI character, not DEM hillshade. QA-037 low peaks (0.09–0.11) now attributed to scale mismatch (CAMERA_ALT_M=12000, ~14 km footprint vs 173 m assumed). AGL-corrected renders at 150 m: 11/12 accepted, mean 0.1451, 6/12 ≥ 0.15 — improved from QA-037 but below 0.30 same-modal expectation. Altitude sweep 150–800 m: monotonic degradation above 150 m; 800 m = 0/12 accepted. Phase correlation at 5 m/px TRN GSD is validated only for AVP-02 (100–300 m AGL); unsuitable for AVP-03/04 (500–2000 m AGL) without multi-scale matching. **Orthophoto matching remains the correct L2 absolute reset architecture for ingress.** Required actions: (1) multi-scale orthophoto reference extraction for AVP-03/04 altitudes; (2) local neighbourhood search around nominal position; (3) fix km=55 JP2 windowed-read edge case (suitability_score=0.000 at small footprints — TCI source window 17×17 px insufficient); (4) re-run at 150 m AGL with lighting/scale adjustments to approach ≥ 0.30 same-modal threshold. | TRN/Navigation | HIGH — corridor validation incomplete; architecture for higher-altitude AVPs undefined |
