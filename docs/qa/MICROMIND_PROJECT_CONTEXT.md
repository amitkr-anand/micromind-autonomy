...
**Last Updated:** 22 April 2026 (QA-050 — Week 1 Day 1: SRS Compliance Matrix + OI-53/OI-54 closed)  
...
| OI-55 | `cross_track_error_m` not present on `SystemInputs` dataclass. CORRIDOR_VIOLATION structured log event (OI-54 fix, `9d99a75`) cannot include breach magnitude in payload. Fix: add `cross_track_error_m: float = 0.0` to SystemInputs; populate from NavigationManager position estimate before FSM.step() call. Not a frozen file change. Required before §16 Corridor Violation row can be fully closed. | Code | MEDIUM — before HIL |
