...
| OI-44 | Cross-modal TRN threshold calibration decision pending. Blender RGB vs DEM hillshade phase correlation peaks: 0.09–0.11. Current operational threshold 0.15 rejects all 12 frames. Calibrated threshold: 0.091 (P10 of corridor distribution). Programme Director must decide: (a) lower operational `min_peak_value` to 0.091 for cross-modal use, or (b) accept reduced TRN coverage in RGB-DEM cross-modal regime. Must be resolved before NAV-02 HIL gate. | Architecture | HIGH — before NAV-02 HIL |
