...
| OI-30 Phase A + B (run_demo.sh) | ✅ COMMITTED | Gazebo server + GUI (Phase A); PX4 dual-instance launch + EKF2 alignment wait (Phase B). Phase C (run_mission.py) pending. SIL 290/290 ✅ | `afdde74` |
...
| OI-30 | **Phase A + Phase B COMMITTED afdde74 (10 Apr 2026).** Phase A: Gazebo server (baylands, OGRE1, `GZ_ENGINE_NAME=ogre`) + GUI with NVIDIA EGL fix; scene/info service ready-check. Phase B: PX4 instance 0 (Vehicle B, pose 0,0,0.5, port 14540) and instance 1 (Vehicle A, pose 0,5,0.5, `PX4_GZ_STANDALONE=1`, port 14541) launched from `/tmp/px4_inst{0,1}` working dirs; EKF2 alignment polled via `gz topic -e -n 1` on `/fmu/out/vehicle_local_position` (inst 0) and `/px4_1/fmu/out/vehicle_local_position` (inst 1), 60s timeout, exits 1 on timeout. SIL: 290/290 green. **Phase C (run_mission.py wiring) — NEXT TASK (Prompt 3).** | Code | CRITICAL — before any OEM meeting |
