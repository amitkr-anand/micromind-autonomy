...
## 6. Current Programme State (26 April 2026 — updated QA-066)
...
| JL-SENTINEL-EXTRACT — Jammu-Leh Sentinel-2 TCI extraction | ✅ COMPLETE | All 6 priority tiles extracted. docs/data/JAMMU_LEH_SENTINEL_INVENTORY.md committed (QA-066). Coverage: 74°59'59"E–78°19'36"E × 32°24'22"N–34°20'30"N. T43SES/SET/SFS/SFT/SGS/SGT. Total TCI 790 MB. Jammu city (74.87°E) outside left edge by ~11 km; T43SDT needed for Srinagar closure. ZIP source 14 GB — no disk pressure (1.2 TB free). ZIP retention pending Deputy 1 quality validation ruling. | docs + data | COMPLETE |
...
| W1-P09 — PLN-02 R-02 callback-based EW refresh | ✅ CLOSED `168b1d5` — `_ew_refresh_fn()` called at staleness detection point (Option C). After callback, re-read `ew_age_s_post`. If fresh: log `RETASK_EW_MAP_REFRESHED` (INFO). If still stale: log `RETASK_EW_MAP_STALE_PROCEED` (WARNING) and proceed. No spin-wait, no new instance variables, no return type change. `test_adv_01` updated: asserts exactly one outcome event. `test_adv_01b` added: verifies `RETASK_EW_MAP_REFRESHED` when callback updates map. SIL 511/511. | Code | CLOSED |
...
| IT-D6-TIMEOUT-01 — D6 OffboardRecoveryFSM (D2/D3/D6 timeout chain) | ✅ CLOSED `6a30295` — OffboardRecoveryFSM implemented (integration/bridge/offboard_recovery_fsm.py). D2 5s + D3 5s at 1Hz retry. OFFBOARD_UNRECOVERED event + abort_fn() callback on D6. SR-01 compliant. 4 tests / 17 assertions PASS. Compliance matrix D6 row corrected (was incorrectly "Implemented") at fac941c. SIL 532 → 536/536. Deputy 1 rules on gate acceptance. | Code + Test | CLOSED (pending Deputy 1 acceptance) |
| IT-D6-SITL-01 — D6 path SITL integration test | ✅ CLOSED `a3fd35e` — tests/test_it_d6_sitl.py (@pytest.mark.sitl). ARM + OFFBOARD on live PX4 SITL. GCS heartbeat stopped → setpoints stopped → fsm.on_offboard_loss() triggered → D2/D3 SET_MODE attempts fail (link-severed semantics) → D6 fires. G1 OFFBOARD_LOSS ✓ G2 SHM_ENTRY ✓ G3 OFFBOARD_UNRECOVERED (total_elapsed_ms=10001ms, attempts_made=10) ✓ G4 abort_fired ✓ G5 elapsed=10.00s ✓. Evidence: IT_D6_SITL_EVIDENCE_RUN1.md. SIL 536/536 unchanged. Deputy 1 rules on gate acceptance. | Code + Evidence | CLOSED (pending Deputy 1 acceptance) |
| IT-D9-CHAIN-01 — Full D7→D8→D8a→D9 SITL reboot recovery chain | ✅ CLOSED `208a5a1` — G1 PASS 1980 ms (≤3 s), G2 PASS 0 ms (≤15 s), G3 PASS AUTONOMOUS_RESUME_APPROVED, G4 PASS pos_disc=43.678 m. Evidence: `5863020` IT_D9_CHAIN_EVIDENCE_RUN1.md. SRS §8.4 PX4-04. Deputy 1 rules on gate acceptance. Baseline 532/532. | Code + Evidence | CLOSED (pending Deputy 1 acceptance) |
...
| ~~OI-40~~ | **CLOSED** `3230ba6` — SRS_V1_4_DELTA.md committed 26 Apr 2026. §16 Corridor Violation row: Detects=Navigation Manager, Decides/Executes=NanoCorteXFSM (5 states: NOMINAL 297, EW_AWARE 320, GNSS_DENIED 361, NAV_TRN_ONLY 399, SILENT_INGRESS 440), Logs=_log_corridor_violation_event(). OI-55 CLOSED at 3e79805. §16 matrix row updated OPEN→CLOSED. | Architecture/Docs | CLOSED |
...
| ~~OI-56~~ | **CLOSED** `e7d3d42` — `_eta_s: float` added to RoutePlanner as 5th rollback state variable. `_rollback()` now accepts and restores `snap_eta_s_val`. ETA updated from 2-D route length on every successful retask. RETASK_ROLLBACK event emitted on all failure paths with `eta_s_restored` payload. SRS §4.2 Appendix B ROLLBACK action (5) satisfied. `test_r03_eta_rollback` gate PASS. Diagnostic (W2-1b) confirmed: MissionManager holds zero ETA attributes and has no retask() method — ETA lives only in Checkpoint (persistence field) and now in RoutePlanner (live state). | Architecture | CLOSED |
...
---
**SRS_COMPLIANCE_MATRIX.md v4 reconciliation (QA-065, 26 April 2026, commit 3230ba6):**
PLN-02 CLOSED (all R-01..R-06 compliant). EC-01 CLOSED (30-min gate 8e50cbc). §16 Corridor Violation CLOSED (SRS_V1_4_DELTA.md, OI-40 CLOSED, OI-55 CLOSED). App-B REJECTED OPEN→PARTIAL (RETASK_REJECTED event name not in code — Task A code audit finding). IT-D10-GNSS-01 NOT STARTED→PASSED (711bf6d). Dashboard corrected. Requirements totals: 47C/21P/4O/2B/1N/A. §1.2 High-Risk Items reduced to 3 (OI-40 and D10 removed). Push to origin/main: PASS. Orin sync: DEFERRED (node offline).
...
**W2-4 EC-01 status update (25 April 2026):**
EC01_EVIDENCE_RUN1.md submitted to Deputy 1. EC01-G1/G2/G3/G4 all PASS on live SITL.
EC-01 status: PARTIAL → EVIDENCE SUBMITTED — Deputy 1 ruling on gate acceptance pending.
SAL-3 baseline frozen at `57aa994` (W2-3).

**IT-ROLLBACK-01 compliance matrix correction (25 April 2026, `537fab1`):**
SRS_COMPLIANCE_MATRIX.md Appendix B: ROUTING row Impl Status corrected from "Partially Implemented" to "NOT IMPLEMENTED" (TERRAIN_GEN_FAIL path absent). COMMITTING row Impl Status corrected from "Implemented" to "NOT IMPLEMENTED" (COMMIT_FAIL path absent). Implementation completed at `17330fa`. SIL 523 → 526/526. Deputy 1 rules on IT-ROLLBACK-01 gate acceptance.
---
...
| OI-JL-01 | Validate JL TCI pixel quality (cloud cover, dark pixels) for NAV-02 characterisation use — T43SGS notably small at 53 MB; winter imagery. Deputy 1 quality validation ruling required before ZIP deletion. | QA | MEDIUM |
| OI-JL-02 | Extract T43SDT TCI to close Srinagar western gap (~17 km west of T43SET left edge at 75°E). Deputy 1 authorisation required. | Data | LOW |
