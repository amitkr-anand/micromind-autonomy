...
| HIL H-4 — LightGlue Subprocess IPC Bridge | ✅ COMPLETE | Unix socket IPC: server.py (hil-h3 Python 3.10) + client.py (Python 3.11). Orin verified: T1 PASS (ping 0.7ms), T2 PASS (real GPU conf=0.826 match_ms=635ms steady-state), T3 PASS (invalid_coords). IPC overhead 1.0ms on Orin. CUDA fix: libcusparseLt resolved via LD_LIBRARY_PATH injection. Contract: docs/interfaces/L2_LIGHTGLUE_IPC.md. | `26407a1` |
