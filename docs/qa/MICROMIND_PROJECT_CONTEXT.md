...
## 6. Current Programme State (10 April 2026)
...
| OI-38 | PX4-05 Checkpoint v1.2 schema (fcb5106) + PX4-04 reboot detection + D8a gate (787ecd4): SA-01–SA-07 all PASS. EC-02 partially addressed — schema + serialisation + D8a gate complete. EC-03 addressed. SB-5 Phase A closed (7 gates, 297/297 SIL). Phase B gates (SA-08+) pending next prompt. | Code | MEDIUM — SB-5 Phase A closed, Phase B pending |
| OI-39 | EC-07 non-compliance — GNSS Spoof event has no dedicated named log event string. `core/bim/bim.py` `_detect_spoof()` correctly sets `spoof_alert=True` in BIMResult (line 288) but no `log()` call emits a named event (e.g. `GNSS_SPOOF_DETECTED`). §16 requires Mission Manager (trust state consumer) and Demo Tool to be able to subscribe to/audit the detection event. Without a named event string this is not possible. Fix: add `_log.warning("GNSS_SPOOF_DETECTED: ...")` or equivalent structured log call in `_detect_spoof()` (bim.py is frozen — requires explicit unfreeze instruction). See `docs/qa/SB5_EC07_OwnershipVerification.md`. | Code | MEDIUM — correct module, missing log call; fix required before Phase A exit gate |
