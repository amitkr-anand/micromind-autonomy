...
| OI-31 | Demo design session required before SB-5 Phase C — Run 1 (live two-vehicle Gazebo) and Run 2 (150 km interactive simulation) display layout, marker design, fault injection panel, and tile boundary specification must be completed before VIZ-02 sprint begins. Entry condition: SB-5 Phase A and Phase B exit gates confirmed PASS. **Phase C entry condition MET (11 Apr 2026) — Handoff 1 QFR CLOSED, 314/314. OI-31 demo design session is now the active Phase C entry gate and governs VIZ-02 scope.** Handoff 1 process findings: PF-01, PF-02, PF-03 documented in `docs/governance/DEPUTY1_STANDING_NOTES.md`. | Architecture | HIGH — gates Phase C start |
