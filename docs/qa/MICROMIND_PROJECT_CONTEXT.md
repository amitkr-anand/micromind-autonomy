...
## 6. Current Programme State (13 April 2026 — updated QA-033)
...
| OI-42 | Live SITL VIO blocked: `dirt_diffusespecular.png` (Gazebo Classic heightmap diffuse texture) not present in Gazebo Harmonic gz-rendering8. `simulation/worlds/shimla_nav_test.world` texture block removed as workaround (world now loads), but terrain visual renders as uniform sky background — zero Shi-Tomasi features, zero VIO estimates. Fix: provide `simulation/terrain/shimla/media/materials/textures/dirt_diffusespecular.png` (generate from Shimla hillshade or source equivalent OGRE terrain texture). `flat_normal.png` available at PX4-gazebo-classic. Required before Live SITL VIO confidence measurement can proceed. | Code | HIGH — blocks Live SITL VIO verification |
