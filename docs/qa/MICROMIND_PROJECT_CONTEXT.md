...
| OI-33 | Demo world decision required before run_demo.sh is finalised — two_vehicle_sitl.sdf (minimal verification world, eb33572) is a rendering artifact not a demo world. Decision needed: (a) integrate PX4 Baylands world as demo backdrop to show vehicle motion in recognisable terrain, or (b) create a flat custom world with grid/markers for clean OEM presentation. This decision belongs to the demo design session (OI-31) but must be made before OI-30 can be completed. | Architecture | HIGH — blocks OI-30 finalisation |
---
