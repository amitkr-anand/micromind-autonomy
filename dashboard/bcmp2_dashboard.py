...
def _p1_route(ax, run_out: dict) -> None:
    ...
    dk     = _denial_km(sched)
    seed   = run_out.get("seed", 42)

    # ── Terrain underlay from C-3 synthetic DEM ───────────────────────────
    # Air-gap safe: uses bcmp2_terrain_gen.py — no external data.
    try:
        from scenarios.bcmp2.bcmp2_terrain_gen import BCMP2TerrainGen
        from scenarios.bcmp2.bcmp2_scenario import BCMP2Scenario
        import numpy as _np

        sc      = BCMP2Scenario()
        max_km  = run_out.get("max_km", 150.0)
        bearing = _np.radians(6.12)
        N_GRID  = 25
        along   = _np.linspace(0, max_km * 1000, N_GRID)
        across  = _np.linspace(-500, 500, N_GRID)
        elev    = _np.zeros((N_GRID, N_GRID))

        for pi, phase in enumerate(sc.phases):
            tgen   = BCMP2TerrainGen(phase.terrain_params, seed=seed + pi)
            ps, pe = phase.start_km * 1000, phase.end_km * 1000
            a_mask = (along >= ps) & (along <= pe)
            for ai in _np.where(a_mask)[0]:
                al = along[ai]
                for ci, cr in enumerate(across):
                    n = al * _np.cos(bearing) + cr * (-_np.sin(bearing))
                    e = al * _np.sin(bearing) + cr *   _np.cos(bearing)
                    elev[ci, ai] = tgen.elevation_at(n, e)

        E_g = (along[_np.newaxis, :] * _np.sin(bearing)
               + across[:, _np.newaxis] * _np.cos(bearing)) / 1000
        N_g = (along[_np.newaxis, :] * _np.cos(bearing)
               - across[:, _np.newaxis] * _np.sin(bearing)) / 1000

        ax.contourf(E_g, N_g, elev, levels=12, cmap="Greys",
                    alpha=0.18, zorder=0)
        ax.contour(E_g, N_g, elev, levels=6, colors=[CLR_GRID],
                   linewidths=0.3, alpha=0.22, zorder=1)
    except Exception:
        pass   # terrain underlay is best-effort — never blocks the dashboard
    ...
    if states:
        ...
        ax.plot(te, tn, color=CLR_VB, lw=1.5, ls="-.",
                label="Vehicle B (MicroMind)", alpha=0.9, zorder=3)
        ax.plot(ae, an, color=CLR_VA, lw=1.0,
                label="Vehicle A (baseline)", alpha=0.8, zorder=3)
...
def _p6_c2gates(ax, run_out: dict) -> None:
    ...
    sorted_kms = sorted(int(k) for k in gates.keys())
    ...
    for i, km in enumerate(sorted_kms):
        ...
        gate = gates.get(str(km), gates.get(km, {}))
...
def build_bcmp2_dashboard(
    ...
    show:       bool  = False,
    verbose:    bool  = True,
    run_json:   str   = None,   # load existing run JSON instead of re-running
) -> tuple[str, str]:
    """
    Run BCMP-2 dual-track and render 7-panel comparative dashboard.

    Pass run_json=<path> to load an existing run instead of re-running.
    Returns (png_path, html_path).
    """
    if run_json:
        import json as _json
        if verbose:
            print(f"[1/4] Loading run from {run_json} ...")
        with open(run_json) as _f:
            run_out = _json.load(_f)
        seed   = run_out.get("seed", seed)
        max_km = run_out.get("max_km", max_km)
    else:
        if verbose:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--json",   type=str,   default=None,
                        help="Load existing run JSON instead of re-running")
